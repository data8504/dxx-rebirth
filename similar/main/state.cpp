...
static void state_object_rw_to_object(const object_rw *const obj_rw, object &obj)
{
	...
	switch (obj.control_source)
	{
		...
		case object::control_type::ai:
		{
			...
			obj.ctype.ai_info.CURRENT_STATE = build_ai_state_from_untrusted(obj_rw->ctype.ai_info.flags[1]).value_or(ai_static_state::AIS_REST);
			obj.ctype.ai_info.GOAL_STATE = build_ai_state_from_untrusted(obj_rw->ctype.ai_info.flags[2]).value_or(ai_static_state::AIS_REST);
